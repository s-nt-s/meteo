...
ROOT_DIR = Path(realpath(__file__)).absolute().parent
WD = ('L', 'M', 'X', 'J', 'V', 'S', 'D')
    ...
    for dia in tree.findall(".//dia"):
        ...
        for k in (
            ...
            for node in dia.findall(f".//{k}"):
                ...
                p = _trim(n.get('periodo'))
                if mn is not None:
                    _add(f, p, k+"_min", int(mn))
                if mx is not None:
                    _add(f, p, k+"_max", int(mx))
...
class Meteo:
    ...
    def parse_id(id: str | int):
        ...
        if id.isdecimal():
            return int(id)
        m = re.match(r"^http://www\.aemet\.es/xml/municipios/localidad_(\d+)\.xml", id)
        if m:
    ...
    def print_lluvia(self, prob: int, dt: Optional[datetime] = None):
        ...
        def _sort(x: Periodo):
            d, a, z = x.get_periodo()
            return (d, z-a, a, -x.prob_precipitacion, z, x)
